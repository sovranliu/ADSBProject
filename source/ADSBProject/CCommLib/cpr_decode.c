...
LatLong cpr_decode_local(int lat, int lon, double lat_ref, double lon_ref, BYTE loc, BYTE odd_even) {
    ...
    int i=0;
    if (loc == AIRBORNE_POSITION) {
        dlat = 360 / (4 * NZ - odd_even);
        Nb = 17;
    } else {
        dlat = 90 / (4 * NZ - odd_even);
        //有资料上说地面参数为19，但是文档还是17位
        Nb = 17;
    }
    int lat_j = floor(lat_ref / dlat) + floor(0.5 + mod(lat_ref, dlat) / dlat - 1.0 * lat / pow(2, Nb));
    ...
    double lon_j = floor(lon_ref / dlon) + floor(0.5 + mod(lon_ref, dlon) / dlon - 1.0 * lon / pow(2, Nb));
